...
class StuffTests(APITestCase):
    ...
    def test_login_logout(self):
        ...
        response1 = self.client.get(card_url, format='json', )
...
class TokensVerificationTestCase(APITestCase):
    ...
    def test_two_factor_verification_flow(self):
        ...
        response = self.client.post(signin_url, {'username': user.username, 'password': self.password})
        # if it is all good
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class JWTRefreshTokenTestCase(APITestCase):
    ...
    def test_signin_and_refresh_token_flow_cookies(self):
        ...
        signin_response = self.client.post(signin_url, {'username': self.username, 'password': self.password})
        self.assertEqual(signin_response.status_code, status.HTTP_200_OK)

        refresh_response = self.client.post(refresh_url)
        print(BlacklistedToken.objects.all())
        print('First refreshed __rclientid: ', refresh_response.cookies['__rclientid'].value)
        print('Exp time 1: ', token_backend.decode(refresh_response.cookies['__clientid'].value))
        self.assertEqual(refresh_response.status_code, status.HTTP_200_OK)
        ...
        refresh_response = self.client.post(refresh_url)
        print(BlacklistedToken.objects.all())
        print('Second refreshed __rclientid: ', refresh_response.cookies['__rclientid'].value)
        print('Exp time 2: ', token_backend.decode(refresh_response.cookies['__clientid'].value))
        self.assertEqual(refresh_response.status_code, status.HTTP_200_OK)
        ...
        refresh_response = self.client.post(refresh_url)
        print(BlacklistedToken.objects.all())
        print('Third refreshed __rclientid: ', refresh_response.cookies['__rclientid'].value)
        print('Exp time 3: ', token_backend.decode(refresh_response.cookies['__clientid'].value))
        self.assertEqual(refresh_response.status_code, status.HTTP_200_OK)
    ...
    def test_signin_and_refresh_token_flow_body(self):
        ...
        refresh_response = self.client.post(refresh_url, {'refresh': signin_response.cookies['__rclientid'].value})
        print(BlacklistedToken.objects.all())
        print('First refreshed __rclientid: ', refresh_response.cookies['__rclientid'].value)
        print('Exp time 1: ', token_backend.decode(refresh_response.cookies['__clientid'].value))
        self.assertEqual(refresh_response.status_code, status.HTTP_200_OK)
        ...
        refresh_response = self.client.post(refresh_url, {'refresh': refresh_response.cookies['__rclientid'].value})
        print(BlacklistedToken.objects.all())
        print('Second refreshed __rclientid: ', refresh_response.cookies['__rclientid'].value)
        print('Exp time 2: ', token_backend.decode(refresh_response.cookies['__clientid'].value))
        self.assertEqual(refresh_response.status_code, status.HTTP_200_OK)
